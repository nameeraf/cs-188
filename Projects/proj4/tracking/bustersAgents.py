...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for ghostPosDist in livingGhostPositionDistributions:
            pos = ghostPosDist.argMax()
            dist = self.distancer.getDistance(pos, pacmanPosition)
            if dist < minDist:
                minDist = dist
                mostLikelyPos = pos
    
        # second choose an action that minimizes the maze distance to the closest ghost
        minDist = float("inf")
        chosenAction = None
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(successorPosition, mostLikelyPos)
            if dist < minDist:
                minDist = dist
                chosenAction = action
