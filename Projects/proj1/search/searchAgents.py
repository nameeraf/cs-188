...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.startGameState = self.getStartState()
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # First element of tuple stores starting position
        # Second element of tuple stores a tuple for all the corners that have been visited
    ...
    def isGoalState(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        # If the number of corners visited is equal to the number of corners present, we are at the goal state
        num_visited = len(state[1])
        num_present = len(self.corners)

        return num_visited == num_present
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners:
                    if (nextx, nexty) not in cornersVisited:
                        cornersVisited = cornersVisited + ((nextx, nexty),)

                next_state = ((nextx, nexty), cornersVisited)
...
def cornersHeuristic(state, problem):
    ...
    index_of_min = distances.index(min_dist)
    closest_corner = unvisited[index_of_min]

    dist_to_farthest = 0
    for corner in unvisited:
        dist = util.manhattanDistance(corner, closest_corner)

        if dist > dist_to_farthest:
            dist_to_farthest = dist

    return (min_dist + dist_to_farthest)
...
def foodHeuristic(state, problem):
    ...
    for food_pos in foodGrid.asList():
        ...
        x_food_pos = food_pos[0]
        y_food_pos = food_pos[1]

        if foodGrid[x_food_pos][y_food_pos]:
            if dist > max_dist:
                max_dist = dist
